...
**macOS Intel** — 下载并解压 `CloakBrowser-Manager-<version>-x86_64.zip`（免安装便携版），双击其中的 `CloakBrowser Manager.app` 运行。若首次打开被系统拦截，可改点同目录下的 `CloakBrowser Manager.command`，或在终端执行：
...
预编译安装包见 [Releases](https://github.com/tianyiiiii111/CloakBrowser-Manager/releases)（macOS / Windows 均为便携 `.zip`）。
...
- **桌面端**：PyInstaller + pywebview（macOS / Windows 便携 `.zip`，均支持应用内更新）
...
| **macOS Intel** | 解压便携 zip 后使用。双击 `CloakBrowser Manager.app`，或通过右上角 **刷新图标** →「一键更新并重启」。 |
